...
const SERVER = 'https://ms-teams-clone-server.herokuapp.com/';
...
export const connectWithWebSocket = () => {
  ...
  socket.on('group-call-user-left', (data) => {
    webRTCGroupCallHandler.removeInonlineStream(data);
  });
};
...
const handleBroadcastEvents = (data) => {
  switch (data.event) {
    case broadcastEventTypes.ONLINE_USERS:
      const onlineUsers = data.onlineUsers.filter(onlineUser => onlineUser.socketId !== socket.id);
      store.dispatch(dashboardActions.setOnlineUsers(onlineUsers));
      break;
    case broadcastEventTypes.GROUP_CALL_ROOMS:
      const groupCallRooms = data.groupCallRooms.filter(room => room.socketId !== socket.id);
      const onlineGroupCallRoomId = webRTCGroupCallHandler.checkOnlineGroupCall();

      if (onlineGroupCallRoomId) {
        const room = groupCallRooms.find(room => room.roomId === onlineGroupCallRoomId);
        if (!room) {
