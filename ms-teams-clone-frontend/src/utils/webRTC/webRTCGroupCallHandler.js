...
import { setGroupCallOnline, setCallState, callStates, setGroupCallIncomingStreams, clearGroupCallData } from '../../store/actions/callActions';
...
export const connectWithMyPeer = () => {
  myPeer = new window.Peer(undefined, {
    // path: '/peerjs',
    host: 'ms-teams-clone-peer-server.herokuapp.com',
    port: '443',
    secure:true
  });
...
export const createNewGroupCall = () => {
  ...
  store.dispatch(setGroupCallOnline(true));
...
export const removeInonlineStream = (data) => {
...
// if group call is online return roomId if not return false
export const checkOnlineGroupCall = () => {
  if (store.getState().call.groupCallOnline) {
